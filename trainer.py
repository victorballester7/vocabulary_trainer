...
class word:
  def __init__(self, type: str, eng_word: list[str],
...
def read_data(file_words: str, file_occur: str):
  ...
  for line in L1:
    ...
    X = word(line[0], eng.split(sep), esp.split(sep), L2[len(Words)])
...
def guess_word(n: int, file_occur: str, count: int, t: str):
  if t == 'b':
    ...
    s = t
  # s[::-1] is to revert the order of the string t. That is: 'es' --> 'se'
  # and 'se' --> 'es'.
  r = s[::-1]
    ...
    if (s == 'se'):
      ...
      guess = input(
          "Guess {c} word ({c1} word/s):   {c2}".format(
              ...
              c1=nWords,
              c2=first_char))  # .lower() is to lowercase the word
...
def game(file_occur: str, file_incor: str):  # the game
  ...
  print("1- Spanish -> English (type 'se')\n2- English -> Spanish (type 'es')\n3- Both types (type 'b')\n")
  try:
    t = input()
  except KeyboardInterrupt:
    game_exit(0)
